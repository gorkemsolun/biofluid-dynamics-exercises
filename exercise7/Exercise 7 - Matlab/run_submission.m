function run_submission()
% Generate the six submission artefacts for Gorkem_Solun using the
% combo_K10 configuration (no GUI, no dialogs).

close all
clc

if ~isfolder('Figures')
    mkdir('Figures')
end

%% Simulation parameters (mirror competition.m)
t = 0;
tout = 0;
dtout = 0.025;
tend = 20;

fontsize1 = 12;
Vrbc = 55e-18;
N = 11;
invivo = 0.0;
Ledge = 15e-06;

%% File naming
name = 'Gorkem_Solun';
Path = cd;
if contains(Path, '/')
    Folder = [Path '/Figures/'];
else
    Folder = [Path '\Figures\'];
end

%% Build network
disp('...calling Matlab function create_network.m');
[p, c] = create_network(N, Ledge);

np = size(p,1);
nc = size(c,1);

% Connection distance from inlet (kept for parity with competition.m)
i = 1; gi = 0; gp = repmat(-1, np, 1);
while ~isempty(i)
    gp(i) = gi; gi = gi + 1;
    ic = any(ismember(c(:,1:2), i), 2);
    i = setdiff(unique(c(ic,1:2)), find(gp >= 0));
end

% Variable-capillary mask (top-left half)
xm = max(p(:,1));
ym = max(p(:,2));
l = cross([xm 0 1], [0 ym 1]);
n = l(1)*p(:,1) + l(2)*p(:,2) + l(3);
indp = find(n >= 0);
valc = find(any(ismember(c(:,1:2), indp), 2))';

%% Selected configuration (combo_K10 -- 17.02% in headless run)
constriction = [123 124 10 122 121 56 57 45 34 189];
dilation     = [3 4 15 16 17 26 27 28 38 74];

% Save Setting.mat in the same format the GUI would have
save([Folder name '_Setting.mat'], 'constriction', 'dilation');

%% Apply changes (intersect with valc enforces the rule)
dilation = intersect(dilation, valc);
constriction = intersect(constriction, valc);

if ~isempty(constriction)
    c(constriction, 8) = c(constriction, 8) * 0.6;
end
if ~isempty(dilation)
    c(dilation, 8) = c(dilation, 8) * 1.4;
end

%% Figures
scrsz = get(0, 'ScreenSize');

f1 = figure(1);
set(f1,'OuterPosition',[20 50 (scrsz(3)/2-40) (2*(scrsz(4)/3))]);
f2 = figure(2);
set(f2,'OuterPosition',[(scrsz(3)/2+40) 50 (scrsz(3)/2-40) (2*(scrsz(4)/3))]);

%% RBC setup
bifurcationlaw = 1;
markerstart = 0;
markerduration = 1100;

H = 0.25;
nr = floor((H*nc*Ledge*pi*mean(c(:,8))^2)/Vrbc);
Htmax = 1;

load('randcon')
load('randleng')

r = zeros(nr, 6);
for i = 1:nr
    r(i,3) = floor(randcon(i)*nc) + 1;
    c(r(i,3),4) = c(r(i,3),4) + 1;
    x1 = p(c(r(i,3),1),1); y1 = p(c(r(i,3),1),2);
    x2 = p(c(r(i,3),2),1); y2 = p(c(r(i,3),2),2);
    L = c(r(i,3),6);
    r(i,4) = randleng(i)*L;
    r(i,1) = x1 + r(i,4)/L*(x2-x1);
    r(i,2) = y1 + r(i,4)/L*(y2-y1);
    r(i,5) = -1;
end

%% Tube/discharge hematocrit + initial network solve
c(:,7) = c(:,4)*Vrbc ./ (c(:,6).*pi.*c(:,8).^2);
c(:,9) = compute_discharge_from_tube(c(:,7), 2*c(:,8)*1e6, invivo);
vf = c(:,9) ./ c(:,7);
vf(vf < 1) = 1;
c(:,10) = vf;

disp('...calling Matlab function compute_network');
[p, c] = compute_network(p, c, 0, invivo);

p(:,6) = 0;
p(:,7) = 0;

Thistory = 0;
Hhistory = mean(c(:,7));
Dhistory = std(c(:,7));

disp('...calling Matlab function bifurcation_time');
r(:,6) = bifurcation_time(r, p, c) + t;
tnext = min(r(:,6));
irnext = find(r(:,6) == tnext, 1);
icnext = r(irnext, 3);

passagetime = zeros(100, 2);

disp('...calling Matlab function plot_network');
figure(1)
plot_network(r, p, c);

disp([sprintf('\nThe largest vessel has a diameter of ') num2str(max(c(:,8)*2)*1e6) ' microns.']);
disp(['The smallest vessel has a diameter of ' num2str(min(c(:,8)*2)*1e6) ' microns.']);
numOfChanges = sum(c(:,8) ~= 2.5e-06);
disp(['There was a total of ' num2str(numOfChanges) sprintf(' changed vessel diameters.\n')]);

binDelta = 0.2;
binStart = H - binDelta/2;
binEnd   = H + binDelta/2;
binCount = 0;
binCountAverage = 0;
avCount = 0;

for i = 1:nc
    if (c(i,7) >= binStart) && (c(i,7) <= binEnd)
        binCount = binCount + 1;
    end
end

binCountHistoryT = 0;
binCountHistoryH = binCount;
binCount = 0;

%% Main loop (no per-step plot)
while (t < tend)
    dt = min(tout, tnext) - t;
    dt = max(1e-6, dt);
    t = t + dt;

    ic = r(:,3);
    x1 = p(c(ic,1),1); y1 = p(c(ic,1),2);
    x2 = p(c(ic,2),1); y2 = p(c(ic,2),2);
    L = c(ic, 6);
    v = c(ic, 5);

    vf = c(ic, 9) ./ c(ic, 7);
    vf(vf < 1) = 1;
    c(ic, 10) = vf;
    v = v .* c(ic, 10);

    r(:,4) = min(L, max(0, r(:,4) + dt*v));
    r(:,1) = x1 + r(:,4)./L.*(x2-x1);
    r(:,2) = y1 + r(:,4)./L.*(y2-y1);

    if (t >= tnext)
        if (v(irnext) > 0)
            bifurcationnode = c(icnext, 2);
        else
            bifurcationnode = c(icnext, 1);
        end

        if (bifurcationnode == np)
            bifurcationnode = 1;
            if (r(irnext,5) > 0)
                pt = round(t - r(irnext,5));
                pt = max(1, pt);
                passagetime(pt, 2) = passagetime(pt, 2) + 1;
            end
            pt = min(floor(t + 1), 100);
            if (t >= markerstart && t <= markerstart + markerduration)
                r(irnext, 5) = t;
                passagetime(pt, 1) = passagetime(pt, 1) + 1;
            else
                r(irnext, 5) = -1;
            end
        end

        vBulk = 0;
        newpipe = icnext;
        for j = 1:nc
            if (c(j,1) == bifurcationnode)
                vBulkPipe = -c(j,5);
                Ht = c(j,7);
                if (vBulkPipe < vBulk && Ht < Htmax)
                    newpipe = j; vBulk = vBulkPipe;
                    r(irnext,1) = p(c(newpipe,1),1);
                    r(irnext,2) = p(c(newpipe,1),2);
                    r(irnext,3) = newpipe;
                    r(irnext,4) = 0;
                end
            end
            if (c(j,2) == bifurcationnode)
                vBulkPipe = c(j,5);
                Ht = c(j,7);
                if (vBulkPipe < vBulk && Ht < Htmax)
                    newpipe = j; vBulk = vBulkPipe;
                    r(irnext,1) = p(c(newpipe,2),1);
                    r(irnext,2) = p(c(newpipe,2),2);
                    r(irnext,3) = newpipe;
                    r(irnext,4) = c(newpipe, 6);
                end
            end
        end

        if (newpipe == icnext)
            r(irnext,4) = r(irnext,4) - 0.01*sign(c(icnext,5))*c(icnext,6);
        else
            c(icnext,4) = c(icnext,4) - 1;
            c(newpipe,4) = c(newpipe,4) + 1;
            c(icnext,7) = c(icnext,4)*Vrbc ./ (c(icnext,6).*pi.*c(icnext,8).^2);
            c(newpipe,7) = c(newpipe,4)*Vrbc ./ (c(newpipe,6).*pi.*c(newpipe,8).^2);
            c(icnext,9) = compute_discharge_from_tube(c(icnext,7), 2*c(icnext,8)*1e6, invivo);
            c(newpipe,9) = compute_discharge_from_tube(c(newpipe,7), 2*c(newpipe,8)*1e6, invivo);
            [p, c] = compute_network(p, c, t, invivo);
        end

        r(:,6) = bifurcation_time(r, p, c) + t;
        tnext = min(r(:,6));
        irnext = find(r(:,6) == tnext, 1);
        icnext = r(irnext, 3);

    else
        tout = tout + dtout;
        for i = 1:nc
            p(c(i,1),6) = p(c(i,1),6) + dtout*c(i,7)/p(c(i,1),4);
            p(c(i,2),6) = p(c(i,2),6) + dtout*c(i,7)/p(c(i,2),4);
        end
        p(:,7) = p(:,7) + dtout*p(:,5);

        Thistory = [Thistory t];
        Hhistory = [Hhistory mean(c(:,7))];
        Dhistory = [Dhistory std(c(:,7))];

        for i = 1:nc
            if (c(i,7) >= binStart) && (c(i,7) <= binEnd)
                binCount = binCount + 1;
            end
        end

        binCountHistoryT = [binCountHistoryT t];
        binCountHistoryH = [binCountHistoryH binCount];
        binCount = 0;

        if (t > 10)
            avCount = avCount + 1;
            binCountAverage = (binCountAverage*(avCount-1) + binCountHistoryH(end))/avCount;
        end
    end
end

%% Final plots (mirror competition.m saveas blocks)
figure(1);
f1 = figure(1);
plot_network(r, p, c);
set(f1,'OuterPosition',[scrsz(3)*0.1 scrsz(4)*0.1 scrsz(3)*0.8 scrsz(4)*0.8]);
for j = 1:nc
    xvc = p(c(j,1),1) + (p(c(j,2),1) - p(c(j,1),1))/2;
    yvc = p(c(j,1),2) + (p(c(j,2),2) - p(c(j,1),2))/2;
    if (2*c(j,8) == 5e-06)
        text(xvc,yvc,num2str(j),'HorizontalAlignment','center',...
            'Color','black','BackgroundColor',[.9 .9 .9]);
    elseif (2*c(j,8) > 5e-06)
        text(xvc,yvc,num2str(j),'HorizontalAlignment','center',...
            'Color','black','BackgroundColor','green');
    else
        text(xvc,yvc,num2str(j),'HorizontalAlignment','center',...
            'Color','black','BackgroundColor','red');
    end
end

figure(2);
plot(binCountHistoryT, binCountHistoryH);
xlim([0 20]); ylim([0 nc]);
xlabel('time t [s]', 'FontSize', fontsize1);
ylabel({'number of vessels with a hematocrit';...
    ['between ' num2str(H-binDelta/2) ' and ' num2str(H+binDelta/2)]}, 'FontSize', fontsize1);
text(12, nc*0.8, 'temporal averaging active', 'FontSize', fontsize1);
text(12, nc*0.7, ['current bin count: ' num2str(binCountHistoryH(end))], 'FontSize', fontsize1);
text(12, nc*0.6, ['average bin count: ' num2str(binCountAverage)], 'FontSize', fontsize1);
saveas(f2, [Folder name '_Bin_Count.png']);

p(:,6:7) = p(:,6:7)/t;

fprintf('\n');
disp('...plot the time-averaged hematocrit');
phandle1 = plot_time_averaged_flow(c, p, 'Htavg');
if ~isempty(constriction)
    plot([p(c(constriction,1),1) p(c(constriction,2),1)]', ...
         [p(c(constriction,1),2) p(c(constriction,2),2)]','r-','linewidth',3);
end
if ~isempty(dilation)
    plot([p(c(dilation,1),1) p(c(dilation,2),1)]', ...
         [p(c(dilation,1),2) p(c(dilation,2),2)]','g-','linewidth',3);
end
saveas(gcf, [Folder name '_Hematocrit.png']);

disp('...plot the time-averaged flow');
phandle2 = plot_time_averaged_flow(c, p, 'favg');
if ~isempty(constriction)
    plot([p(c(constriction,1),1) p(c(constriction,2),1)]', ...
         [p(c(constriction,1),2) p(c(constriction,2),2)]','r-','linewidth',3);
end
if ~isempty(dilation)
    plot([p(c(dilation,1),1) p(c(dilation,2),1)]', ...
         [p(c(dilation,1),2) p(c(dilation,2),2)]','g-','linewidth',3);
end
saveas(gcf, [Folder name '_Flow.png']);

f5 = figure(5);
set(f5,'OuterPosition',[1 scrsz(4)/2+1 scrsz(3)/3 scrsz(4)/2-20]);
plot(Thistory, Hhistory); hold on
plot(Thistory, Hhistory + Dhistory, 'r');
plot(Thistory, Hhistory - Dhistory, 'r');
xlim([0 20]); ylim([-0.2 1.2]);
xlabel('time [s]', 'FontSize', fontsize1);
ylabel('average tube hematocrit [-]', 'FontSize', fontsize1);
saveas(f5, [Folder name '_Standard_Deviation.png']);

Percentage_bincount = binCountAverage / nc * 100;
save([Folder name '_Results.mat'], 'Percentage_bincount')

fprintf('\n');
disp(['Congrats! You achieved a desirable tube hematocrit in ' ...
    num2str(binCountAverage/nc*100) '% of your vessels!']);
fprintf('\n');
end
